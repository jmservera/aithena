...
2. **Start the production stack**:
   ...
   `./start.sh` is generated by the installer and applies the correct compose chain for your selected environment, GPU, SSL, and topology options. In production, that normally means `docker-compose.yml` plus `docker/compose.prod.yml` and any chosen overlays.

   If you need to run Compose manually:

   ```bash
   docker compose -f docker-compose.yml -f docker/compose.prod.yml up -d
   ```

   > **Named volumes:** routine production setup no longer requires `scripts/init-volumes.sh`. Docker creates the managed volumes automatically. The only required host path is the document library mount configured by `BOOKS_PATH`.

3. **Optional local source rebuild**:
   ```bash
   ./buildall.sh
   ```

   Use this only when you are building images from the checked-out source instead of pulling the published production images. `buildall.sh` auto-discovers Python services, prepares them with `uv sync`, builds `aithena:base`, and then runs `docker compose up --build -d`.
...
```bash
./start.sh
# Reuses the installer-generated compose chain
```
...
- **Volume permission error**: Verify `BOOKS_PATH` and the installer-created auth directory are writable by Docker, then restart the stack
