...
**v2.2.0 — Installer improvements:** The setup process is streamlined with the Python-based installer (`python3 -m installer`), which guides through library path, admin credentials, GPU acceleration, SSL, and topology in ≤6 prompts. The generated `start.sh` script launches the Docker Compose stack. See [v2.2.0 Release Notes](release-notes/v2.2.0.md).
