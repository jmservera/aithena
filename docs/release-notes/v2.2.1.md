# Aithena v2.2.1 Release Notes — Volume Migration & Test Reliability
...
Aithena v2.2.1 is a maintenance patch release that supersedes the published v2.2.0 tag and is focused on protecting production deployments during the prod-overlay volume migration and improving test reliability. The release does not add a new remote bootstrap script; it updates the existing deployment documentation and compose path to reflect the current repo state.
...
5. **E2E skeleton suites (#1623)** — Adds deterministic coverage scaffolding for thumbnails, document downloads, and error-recovery scenarios.
...
- **#1623** — `test(e2e): Implement skeleton suites` — Captures deterministic thumbnail, download, and recovery coverage scenarios for future test expansion.
...
After pulling v2.2.1 images, run these checks:
...
For detailed technical information, see [CHANGELOG.md](https://github.com/jmservera/aithena/blob/main/CHANGELOG.md) and [v2.2.0 Test Report](../test-reports/v2.2.0.md) and the v2.2.1 CHANGELOG entry.
