...
### 2026-03-17T12:05Z — Upgraded ESLint v8 → v9 with flat config (#345)

- Successfully migrated aithena-ui from ESLint v8.45.0 to v9.39.4 with flat config format
- **Key changes:**
  - Replaced `.eslintrc.cjs` (legacy CommonJS config) with `eslint.config.js` (ESLint 9 flat config using ES modules)
  - Upgraded `typescript-eslint` to v8.57.1 (replaces separate `@typescript-eslint/eslint-plugin` v6 and `@typescript-eslint/parser` v6 packages)
  - Upgraded `eslint-plugin-react-hooks` from v4.6.0 to v5.2.0 (ESLint 9 compatible)
  - Added new dependencies: `@eslint/js` v9.39.4, `globals` v15.15.0
  - Removed `--ext ts,tsx` flag from lint script (ESLint 9 automatically detects file types from config)
- **Migration pattern for flat config:**
  - Import configs as objects: `import js from '@eslint/js'; import tseslint from 'typescript-eslint';`
  - Use `tseslint.config()` wrapper function to merge configs
  - Spread TypeScript configs: `...tseslint.configs.recommended`
  - Define file-specific overrides with `files: ['**/*.{ts,tsx}']`
  - Use `ignores: ['dist']` instead of `ignorePatterns`
- **Gotcha:** Had to add `@testing-library/dom` as explicit dependency (was previously peer dep), otherwise Vitest tests fail with "Cannot find module '@testing-library/dom'"
- **Gotcha:** Used `--legacy-peer-deps` for npm installs to avoid peer dependency conflicts during transition (old v6 packages conflict with ESLint 9)
- **Validation:** All 180 tests passed across 18 test files, build successful, lint clean
- **Outcome:** Created PR #415 against dev branch
