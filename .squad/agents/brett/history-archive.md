...
- **2026-06-13T15:31:32+00:00 — Legacy scripts migration posture:** Treat `./manage.sh` as the canonical day-2 operator entrypoint and keep root-level `scripts/` only as deprecated bridges for specialized runbooks (BCDR, Solr migration, offline export) until the next major version removes them.
- **2026-06-13T15:31:32+00:00 — Legacy scripts migration posture:** Treat `./manage.sh` as the canonical day-2 operator entrypoint (PR #1758, pending merge) and keep root-level `scripts/` only as deprecated bridges for specialized runbooks (BCDR, Solr migration, offline export) until the next major version removes them.

- **2026-06-13T18:06:26+00:00 — Offline package dedupe pattern:** Air-gapped installer bundles should derive runtime images from `docker compose config --images`, ship a manifest of tarball → source tag → compose tags, and collapse the four Solr build aliases (`aithena-solr`, `aithena-solr2`, `aithena-solr3`, `aithena-solr-init`) into one exported image that the installer retags on the target host.
- **2026-06-13T15:37:35+00:00 — Shared build-service discovery:** Keep Dockerfile-based service discovery in `scripts/lib/build-services.sh` and have both `buildall.sh` and `manage.sh build` source it. Filter `uv sync` prep to discovered directories that also contain `pyproject.toml`, so new `src/<service>/Dockerfile` services are auto-detected without accidentally treating infra-only images like `src/solr` as Python prep targets.

- **2026-06-13T19:02:09+00:00 — Shared Python Docker base:** `Dockerfile.base` can safely unify the standard Python services (`document-lister`, `document-indexer`, `solr-search`) around Python 3.12 slim, `uv`, shared Debian tooling, and a reusable healthcheck script, but `embeddings-server` should stay on its specialized GHCR base because it carries heavyweight model/OpenVINO layers and drift-validation gates from #1662.
