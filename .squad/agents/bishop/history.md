...
- 2026-06-07: #1344 is not closable from unpaired readiness data. A local Solr 9.7/`VECTOR_QUANTIZATION=none` run can validate benchmark auth and corpus availability, but closure still requires same-host Solr 10 float32 vs int8 (Aithena's `VECTOR_QUANTIZATION=int8` maps to Solr schema `bits="7"`) reports, `compare_quantization.py`, and measured Solr memory samples from the same corpus.
