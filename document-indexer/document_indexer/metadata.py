...
YEAR_RANGE_PATTERN = re.compile(
    r"(?P<start>1[5-9]\d{2}|20\d{2})\s*-\s*(?P<end>1[5-9]\d{2}|20\d{2})"
)
...
def strip_author_from_title(title: str, author: str | None) -> str:
    ...
    if title_tokens[-len(author_tokens) :] == author_tokens:
        ...
        return " ".join(raw_tokens[: -len(author_tokens)]).strip(" -") or clean_text(
            title
        )
    if title_tokens[: len(author_tokens)] == author_tokens:
        raw_tokens = clean_text(title).split()
...
def extract_metadata(
    file_path: str, base_path: str = "/data/documents/"
) -> dict[str, Any]:
    path = Path(file_path)
    base = Path(base_path)
    ...
    folder_parts = (
        relative_path.parent.parts if relative_path.parent != Path(".") else ()
    )
    ...
    folder_path = (
        "" if relative_path.parent == Path(".") else relative_path.parent.as_posix()
    )
