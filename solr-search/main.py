...
# ---------------------------------------------------------------------------
# Admin — /v1/admin/queue and document management endpoints
# ---------------------------------------------------------------------------


def _get_admin_queue_documents() -> dict[str, list[dict[str, Any]]]:
    """Scan Redis for document-lister queue keys and split by state.

    Keys follow the pattern ``/{queue_name}/*`` and values are JSON objects
    written by the document-lister service.
    """
    try:
        pool = _get_redis_pool()
        client = redis_lib.Redis(connection_pool=pool)
        pattern = f"/{settings.admin_queue_name}/*"
...
@app.get("/v1/admin/queue/", include_in_schema=False, name="admin_queue_slash")
@app.get("/v1/admin/queue", name="admin_queue")
def admin_queue() -> dict[str, Any]:
    """Return document queue state for the admin dashboard.

    Lists all documents discovered by the document-lister, grouped by their
    processing state (queued / processed / failed).  Each document entry
    includes an ``id`` field which is the URL-safe base64 encoding of its
    Redis key and can be used with the requeue endpoint.
    """
    docs = _get_admin_queue_documents()

    def _annotate(doc_list: list[dict[str, Any]]) -> list[dict[str, Any]]:
        result = []
        for doc in doc_list:
            entry = {k: v for k, v in doc.items() if not k.startswith("_")}
            entry["id"] = _encode_queue_key(doc["_redis_key"])
            result.append(entry)
        return result

    return {
        "total": len(docs["queued"]) + len(docs["processed"]) + len(docs["failed"]),
        "queued": len(docs["queued"]),
        "processed": len(docs["processed"]),
        "failed": len(docs["failed"]),
        "queued_documents": _annotate(docs["queued"]),
        "processed_documents": _annotate(docs["processed"]),
        "failed_documents": _annotate(docs["failed"]),
    }


@app.post("/v1/admin/documents/{document_id}/requeue", name="admin_requeue_document")
def admin_requeue_document(document_id: str) -> dict[str, Any]:
    """Requeue a single failed or queued document.

    Deletes the Redis entry so the document-lister will re-discover and
    re-enqueue the document on its next scan.
    """
    redis_key = _decode_queue_key(document_id)
    try:
        pool = _get_redis_pool()
        client = redis_lib.Redis(connection_pool=pool)
    ...
    return {"status": "requeued", "id": document_id}


@app.post("/v1/admin/documents/requeue-failed", name="admin_requeue_failed")
def admin_requeue_failed() -> dict[str, Any]:
    """Requeue all failed documents.

    Deletes the Redis entries for every document in the failed state so the
    document-lister will re-discover them on its next scan.
    """
    docs = _get_admin_queue_documents()
    failed = docs["failed"]
    if not failed:
    ...
    try:
        pool = _get_redis_pool()
        client = redis_lib.Redis(connection_pool=pool)
        keys = [doc["_redis_key"] for doc in failed]
        client.delete(*keys)
    except redis_lib.exceptions.ConnectionError as exc:
        raise HTTPException(status_code=503, detail="Cannot connect to Redis") from exc

    return {"status": "ok", "requeued": len(failed)}


@app.delete("/v1/admin/documents/processed", name="admin_clear_processed")
def admin_clear_processed() -> dict[str, Any]:
    """Clear all processed documents from the Redis queue namespace.

    Removes Redis entries for every processed document.  The document-lister
    will re-discover and re-enqueue them on its next scan.
    """
    docs = _get_admin_queue_documents()
    ...
    try:
        pool = _get_redis_pool()
        client = redis_lib.Redis(connection_pool=pool)
        keys = [doc["_redis_key"] for doc in processed]
        client.delete(*keys)
    except redis_lib.exceptions.ConnectionError as exc:
        raise HTTPException(status_code=503, detail="Cannot connect to Redis") from exc

    return {"status": "ok", "cleared": len(processed)}
