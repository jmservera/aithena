...
import { useSearch, BookResult, SearchFilters } from "./hooks/search";
...
function App() {
  ...
  const [pdfBook, setPdfBook] = useState<BookResult | null>(null);
  const {
    searchState,
    results,
    facets,
    ...
    <div className="App">
      ...
      <main className="search-main">
        ...
        <section className="search-results">
          ...
          {results.map((book) => (
            <BookCard key={book.id} book={book} onOpenPdf={setPdfBook} />
          ))}
        </section>
      ...
      {pdfBook && (
        <PdfViewer result={pdfBook} onClose={() => setPdfBook(null)} />
      )}
    </div>
  );
}
