...
def scalar_quantized_vector_field_type(
    ...
    Solr 10 uses ``ScalarQuantizedDenseVectorField`` with explicit ``bits``.
    Lucene's scalar-quantized codec accepts 4 or 7 bits only; 7 bits is the
    signed-byte mode and preserves the existing ``VECTOR_QUANTIZATION=int8``
    byte-vector storage intent. During the Solr 9 compatibility window, the
    equivalent schema is ``DenseVectorField`` with ``vectorEncoding=BYTE`` and
    any ``bits`` value is ignored.
    """
    solr_10 = is_solr_10(solr_url=solr_url, auth=auth)
    if solr_10:
        if bits is None:
            raise ValueError("Solr 10 ScalarQuantizedDenseVectorField requires explicit bits")
        if bits not in (4, 7):
