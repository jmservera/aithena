...
import { tierHealthColor, formatBytes, formatDuration } from '../hooks/backups';
import type { TierStatus } from '../hooks/backups';

const mockTiers: TierStatus[] = [
  {
    tier: 'critical',
    last_backup: '2025-01-15 10:00',
    age_hours: 2,
    rpo_hours: 4,
    size: 1048576,
    status: 'completed',
  ...
  {
    tier: 'high',
    last_backup: '2025-01-14 08:00',
    age_hours: 20,
    rpo_hours: 12,
    size: 5242880,
    status: 'completed',
  },
  { tier: 'medium', last_backup: null, age_hours: 0, rpo_hours: 24, size: 0, status: 'pending' },
];
...
function createMockFetch() {
  return vi.fn().mockImplementation((url: string) => {
    if (typeof url === 'string' && url.includes('/v1/admin/backups/status')) {
      return Promise.resolve({ ok: true, json: () => Promise.resolve({ tiers: mockTiers }) });
    }
    if (typeof url === 'string' && url.includes('/v1/admin/backups')) {
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ backups: mockBackups, total: mockBackups.length }),
      });
    }
...
describe('BackupDashboardPage', () => {
  ...
  it('renders tier status cards after loading', async () => {
    renderPage();
    await waitFor(() => {
      expect(screen.getByText('Critical', { selector: 'h4' })).toBeInTheDocument();
    });
    expect(screen.getByText('High', { selector: 'h4' })).toBeInTheDocument();
    expect(screen.getByText('Medium', { selector: 'h4' })).toBeInTheDocument();
  });

  it('renders backup history table', async () => {
    renderPage();
    await waitFor(() => {
      expect(screen.getByText('Backup History', { selector: 'h3' })).toBeInTheDocument();
    });
  ...
  it('shows Backup Now section with tier selector', async () => {
    renderPage();
    await waitFor(() => {
      expect(screen.getByText('Backup Now', { selector: 'h3' })).toBeInTheDocument();
    });
  ...
  it('can sort backup history by clicking column header', async () => {
    ...
    const tierHeader = screen.getByText('Tier', { selector: 'th' });
  ...
  it('shows backup details in restore wizard select step', async () => {
    ...
    await user.click(screen.getByText('Restore'));
    expect(
      screen.getByText('You are about to restore from the following backup:')
    ).toBeInTheDocument();
  ...
  it('can navigate through restore wizard steps', async () => {
    ...
    expect(screen.getByText('The following components will be restored:')).toBeInTheDocument();
    await user.click(screen.getByText('Next'));
    expect(
      screen.getByText('This will overwrite current data with the backup contents.')
    ).toBeInTheDocument();
  });
